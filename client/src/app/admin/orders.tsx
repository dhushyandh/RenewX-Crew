...
import { ScrollView, Text, TouchableOpacity, View, ActivityIndicator, RefreshControl, Modal, FlatList, Pressable } from "react-native";
...
export default function AdminOrders() {
    ...
    const fetchOrders = async () => {
        try {
            ...
            console.log('Failed to fetch orders from API, using dummy orders:', error);
            setOrders(dummyOrders.map((order: any) => ({
                ...order,
                user: dummyUser
            })));
    ...
    const updateStatus = async (newStatus: string) => {
        ...
        try {
            ...
            setOrders(orders.map((order: any) => order._id === selectedOrder._id ? { ...order, orderStatus: newStatus } : order));
            Toast.show({
                type: 'success',
                text1: 'Status Updated',
                text2: `Order status set to ${newStatus}`
            });
        } catch (error) {
            console.log('API status update failed, updated locally:', error);
            setOrders(orders.map((order: any) => order._id === selectedOrder._id ? { ...order, orderStatus: newStatus } : order));
        } finally {
    ...
    return (
        <View className="flex-1 bg-surface">
                    ...
                    orders.map((order: any) => (
                        <View key={order._id} className="bg-white p-4 rounded-xl shadow-sm mb-4 border border-gray-100">
                            <View className="flex-row justify-between mb-2">
                                ...
                                <Text className="text-secondary text-xs">{order.createdAt ? new Date(order.createdAt).toLocaleDateString() : ''}</Text>
                            </View>
                            ...
                            <View className="mb-3">
                                ...
                                {order.items?.map((item: any) => (
                                    <View key={item._id || item.name} className="flex-row justify-between mb-1">
            ...
            <Modal visible={statusModalVisible} animationType="fade" transparent>
                <Pressable onPress={() => setStatusModalVisible(false)} className="flex-1 justify-end bg-black/50">
